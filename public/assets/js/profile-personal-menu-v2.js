(function () {
  ...
  function setStatus(text, state) {
    $$('[data-profile-autosave-status]').forEach((node) => {
      ...
      node.dataset.autosaveState = state || 'ready';
    });
  }

  function hideLegacyMenuBlock() {
    $$('.preference-group').forEach((group) => {
      const text = group.querySelector('summary')?.textContent || '';
      if (!text.includes('Món ghim & gần đây')) return;
      group.classList.add('legacy-menu-preferences-hidden');
      $$('input, select, textarea, button', group).forEach((field) => field.disabled = true);
    });
  }

  function renumberMenuRows(root) {
    $$('[data-personal-menu-sortable]', root || document).forEach((tbody) => {
  ...
  function finishDrag(form, shouldSave) {
    const hadDrag = Boolean(dragRow || dragReadyRow);
    if (dragRow) {
      dragRow.classList.remove('is-dragging');
      dragRow.removeAttribute('draggable');
    }
    if (dragReadyRow) dragReadyRow.removeAttribute('draggable');
    dragRow = null;
    dragReadyRow = null;
    renumberMenuRows(form);
    if (shouldSave && hadDrag) queueAutosave(form, 150);
  }
  ...
  function initDragSorting(form) {
    ...
    tbody.addEventListener('pointerdown', (event) => {
      ...
      if (dragReadyRow) dragReadyRow.setAttribute('draggable', 'true');
    });
    ...
    tbody.addEventListener('dragover', (event) => {
      ...
      if (!after) tbody.appendChild(dragRow);
    ...
    tbody.addEventListener('dragend', () => finishDrag(form, true));
  }
  ...
  async function saveNow(form) {
    ...
    try {
      ...
      if (!response.ok || payload.saved === false) throw new Error(payload.message || 'Không tự lưu được');
  ...
  function queueAutosave(form, delay) {
    ...
    saveTimer = setTimeout(() => saveNow(form), typeof delay === 'number' ? delay : 650);
  }

  function initAutosave(form) {
    form.addEventListener('input', (event) => {
      if (event.target.closest('[data-drag-handle]')) return;
      queueAutosave(form, 650);
    });
    form.addEventListener('change', () => queueAutosave(form, 250));
  ...
  function init() {
    ...
    if (!form || form.dataset.personalMenuReady === '1') return;
    form.dataset.personalMenuReady = '1';
    renumberMenuRows(form);
