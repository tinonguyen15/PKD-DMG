...
<form id="order-new-processing-form" method="post" action="<?= e(url('/orders/new-processing')) ?>" hidden>
  <?= csrf_field() ?>
</form>
...
<section class="panel draft-panel order-open-panel">
  <div class="section-head">
    ...
    <button class="btn primary" type="submit" form="order-new-processing-form">Thêm đơn</button>
  </div>
      ...
      <div class="open-order-list">
        <?php foreach ($activeOrders as $row): ?>
          ...
          <article
            ...
            <?= $isProcessing ? 'data-open-order-url="' . e(url('/orders/create?edit_order_id=' . (int) $row['id'])) . '"' : '' ?>
          >
            <div class="open-order-main">
              <a href="<?= e(url('/orders/' . (int) $row['id'])) ?>"><?= e($row['order_code']) ?></a>
              <span><?= $isCurrentEdit ? 'Đang làm' : e($workflowLabels[$status] ?? $status) ?></span>
            </div>
            ...
            <div class="open-order-bottom">
              ...
              <div class="open-order-actions">
                <?php if ($status === 'sent'): ?>
                  <button class="btn ghost open-order-edit-btn" type="submit" form="<?= e($reopenFormId) ?>" title="Sửa lại đơn đã gửi CN">✎</button>
                <?php endif; ?>
                <button
                  class="btn ghost"
                  type="button"
            ...
            <?php if ($isProcessing): ?>
              <small class="open-order-hint">Click card để làm tiếp</small>
            <?php else: ?>
              <small class="open-order-hint">Bấm ✎ để chuyển về Đang xử lý và sửa lại</small>
            <?php endif; ?>
            <textarea id="<?= e($copyId) ?>" class="active-order-copy" readonly><?= e($row['generated_text'] ?: '') ?></textarea>
          </article>
        <?php endforeach; ?>
        <?php if (!$activeOrders): ?>
          <p class="open-order-empty">Chưa có đơn đang xử lý hoặc đã gửi CN.</p>
        <?php endif; ?>
      </div>
    </div>
  </div>
...
<?php if ($editingOrderId > 0): ?>
  ...
  <form
    ...
    action="<?= e(url('/orders')) ?>"
    data-order-create
    data-order-editing="1"
    data-edit-order-code="<?= e($editingOrderCode ?: ('#' . $editingOrderId)) ?>"
    data-draft-save-url=""
    data-drafts-url=""
  ...
  >
    ...
    <section class="panel order-form-panel">
      <div class="section-head">
        <h2>Làm tiếp đơn <?= e($editingOrderCode ?: ('#' . $editingOrderId)) ?></h2>
...
<?php else: ?>
  <section class="panel order-empty-workspace">
...
<?php foreach ($activeOrders as $row): ?>
  ...
  <form id="active-order-reopen-<?= (int) $row['id'] ?>" class="active-order-reopen-form" method="post" action="<?= e(url('/orders/' . (int) $row['id'] . '/reopen-edit')) ?>">
    <?= csrf_field() ?>
  </form>
<?php endforeach; ?>
